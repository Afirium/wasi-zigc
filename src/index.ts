...
async function buildDirectoryTree(
	files: Generator<ZipItem, void, void>,
): Promise<Directory> {
		...
		parts.forEach(async (part, index) => {
			if (part !== "") {
				if (index === parts.length - 1) {
					...
					currentDirectory = currentDirectory.contents.get(
						parts[index],
					) as Directory;
				}
			}
...
export async function initZigWASI(
	std: Uint8Array,
	zigCode: string,
	debug = false,
): Promise<WASI> {
...
export async function runZigCompiler(
	zigc: BufferSource,
	wasi: WASI,
): Promise<WASI> {
...
export async function runZigOutput(output: ArrayBuffer): Promise<WASI> {
	...
	const fds = [
		...
		new PreopenDirectory(
			".",
			new Map([["input.wasm", new File(new Uint8Array(output))]]),
		),
	];
